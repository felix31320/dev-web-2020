        ...
        <?php
            ...
            if (!empty($_GET['TITRE']) && !empty($_GET['CONTENU']) && !empty($_GET['AUTEUR'])) {
                ...
                if (isset($_GET['send'])) {
                    ...
                    $req2 = $bdd->prepare('INSERT INTO articles (titre, contenu, auteur) VALUES (:titre,:contenu,:auteur)');
                    $req2->execute(array('titre'=>$titre,'contenu'=>$contenu,'auteur'=>$auteur));
                    
                    // echo 'votre compte est envoyer';
                    // $strSQL ='INSERT INTO articles (titre, contenu, auteur) VALUES (\''.$titre.'\', \''.$contenu. '\', \''.$auteur.'\')';
                    // $bdd-> exec($strSQL);
                } else {
