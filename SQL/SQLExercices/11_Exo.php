...
if (!empty($_GET['matiereFilre'])) {
    echo 'à filtrer <br>';
} else {
    echo 'non filtrer <br>';
}

if (!empty($_GET['matiereSelect'])) {

    $strSQL = 'SELECT controle_id, controle_exam, controle_matiere, etudiant_nom, etudiant_prenom, 
    controle.etudiant_id,controle_note,controle_date  
    FROM controle 
    INNER JOIN etudiant ON controle.etudiant_id = etudiant.etudiant_id 
    WHERE controle_matiere=\''.$_GET['matiereSelect'].'\'
    ORDER BY controle_id DESC';

    $req = $bdd->query($strSQL);
} else {
...
<?php
        ...
        echo '<select style="width: 100%;    padding: 2px 5px;" name="matiereSelect" onchange="submit()">';
        echo '<option value="">Sélectionner la matière</option>';
        while ($reponse3 = $req3->fetch()){
            echo '<option value="'.$reponse3['controle_matiere'].'">'.$reponse3['controle_matiere'].'</option>';
        }
        ...
        echo '<input type="hidden" name="filtreSelect" value="1">';
        echo '</select>';
?>
</form>
<?php              
echo '</td>';
echo '<td>etudiant_id</td>';
echo '<td>etudiant_nom</td>';
echo '<td>etudiant_prenom</td>';
echo '<td>controle_note</td>';
echo '<td>controle_date</td>';
echo '</tr>';
$nb =0;
while ($reponse = $req->fetch()){
    ...
    echo '<tr>';
    echo '<td>'.$nb.'</td>';
    echo '<td style="text-align: right">'.$reponse['controle_id'].'</td>';
    echo '<td style="text-align: right">'.$reponse['controle_exam'].'</td>';
    echo '<td>'.$reponse['controle_matiere'].'</td>';
    echo '<td style="text-align: right">'.$reponse['etudiant_id'].'</td>';
    echo '<td style="text-align: left">'.$reponse['etudiant_nom'].'</td>';
    echo '<td style="text-align: left">'.$reponse['etudiant_prenom'].'</td>';
    echo '<td style="text-align: right">'.$reponse['controle_note'].'</td>';
    echo '<td style="text-align: right">'.$reponse['controle_date'].'</td>';
    echo '</tr>';
}
echo '<table>';
$req->closeCursor();
