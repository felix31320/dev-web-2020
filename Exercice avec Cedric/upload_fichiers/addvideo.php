...
if(isset($_POST['btn_insert']))
{
	try
	{
		...
		$path="uploadvideo/".$image_file; //je défini le nom du fichier et son répertoire
		...
		else if($type=='video/mp4') //vérification du format de fichier
		...
		{	
			if(!file_exists($path)) //est-ce que le fichier existe déjà ?
			{
				...
				{
					move_uploaded_file($temp, "uploadvideo/" .$image_file); //je stocke le fichier temporairement
				}
				else
				{
		...
		if(!isset($errorMsg))
		{
			$insert_stmt=$db->prepare('INSERT INTO tbl_video(name,video) VALUES(:fname,:fimage)'); //requête SQL SELECT					
